...
export const UserRegister=createAsyncThunk("UserRegister",async(data)=>{
    try {
       ...
       {
        toast.success("User suceesfully registered");
...
export const userlogin=createAsyncThunk("userlogin",async(data)=>{
    try {
       ...
       if(response.status===200)
       {
        ...
        toast.success("User suceesfully login");
        return response.data;
       } else{
        toast.error(response.response.data.error);
       }
...
// userlogoutfun
export const userlogoutfun=createAsyncThunk("userlogoutfun",async(thunkApi)=>{
    try {
...
export const forgotpasswordvalid=createAsyncThunk("forgotpasswordvalid",async(data)=>{
    try {
       ...
       if(response.status == 200){
        ...
        toast.error("your link expire please generate a new link ")    
    }
    } catch (error) {
        throw error
    }
...
export const usercontact=createAsyncThunk("usercontact",async(data)=>{
    ...
    //    console.log("response",response);
       if(response.status == 200){
        toast.success("Your message is save"); 
        return response.data
    }else{
        toast.error(response.response.data.error); 
    }
...
export const UserSlice=createSlice({
    ...
    extraReducers:(builder)=>{
        ...
        // userlogoutfun
        .addCase( userlogoutfun.pending,(state)=>{
            state.loading=true;
        })
        .addCase(userlogoutfun.fulfilled,(state,action)=>{
            state.loading=false;
            state.UserLogout=[action.payload];
            state.UserLoggedIn=[];
            state.userCartData=[];
        })
        .addCase(userlogoutfun.rejected,(state,action)=>{
            state.loading=false;
            state.error=action.payload;
        })
        // forgotpassword
